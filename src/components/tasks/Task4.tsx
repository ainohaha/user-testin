...
export function Task4({ onComplete, onPrevious, taskNumber, totalTasks, participantId, micStream, screenStream, isRecording, scenario, existingData }: Task4Props) {
  const [completed, setCompleted] = useState(existingData?.completed || '');
  const [clarity, setClarity] = useState(existingData?.clarity ? [existingData.clarity] : [3]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6 space-y-5">
        <div className="space-y-3">
          ...
          <RadioGroup value={completed} onValueChange={setCompleted}>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="yes" id="t4-yes" />
              <Label htmlFor="t4-yes">Yes</Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="no" id="t4-no" />
              <Label htmlFor="t4-no" className="flex-1">
                <span>No</span>
                {showTimeWarning && (
        ...
        <div className="space-y-3">
          <h3>How intuitive was the shop setup process? *</h3>
          <div className="space-y-2 px-2">
            ...
            <div className="flex justify-between items-center relative" style={{ paddingLeft: '0px', paddingRight: '0px' }}>
              {[1, 2, 3, 4, 5].map((num, index) => (
                ...
                >
                  <span className={`text-sm ${clarity[0] === num ? 'text-primary' : 'text-slate-400'}`}>
                    {num}
                  </span>
                </div>
              ))}
        ...
        <div className="space-y-3">
          <Label htmlFor="t4-feedback">
            What did you think about setting up your shop? Was any information missing or confusing? *
          </Label>
          <Textarea
            id="t4-feedback"
