...
export function Task7({ onComplete, onPrevious, taskNumber, totalTasks, participantId, micStream, screenStream, isRecording, scenario, existingData }: Task7Props) {
  const [completed, setCompleted] = useState(existingData?.completed || '');
  const [confidence, setConfidence] = useState(existingData?.confidence ? [existingData.confidence] : [3]);
  ...
  const isComplete = completed !== '' && concerns.trim() !== '';
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6 space-y-5">
        <div className="space-y-3">
          ...
          <RadioGroup value={completed} onValueChange={setCompleted}>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="yes" id="t7-yes" />
              <Label htmlFor="t7-yes">Yes</Label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value="no" id="t7-no" />
              <Label htmlFor="t7-no" className="flex-1">
                <span>No</span>
                {showTimeWarning && (
        ...
        <div className="space-y-3">
          <h3>How easy was it to complete this task? *</h3>
          <div className="space-y-2 px-2">
            ...
            <div className="flex justify-between items-center relative" style={{ paddingLeft: '0px', paddingRight: '0px' }}>
              {[1, 2, 3, 4, 5].map((num, index) => (
                ...
                >
                  <span className={`text-sm ${confidence[0] === num ? 'text-primary' : 'text-slate-400'}`}>
                    {num}
                  </span>
                </div>
              ))}
        ...
        <div className="space-y-3">
          <Label htmlFor="t7-concerns">
            What did you think about scheduling a drop off? Any confusion or missing information? *
          </Label>
          <Textarea
            id="t7-concerns"
